...
import com.ly.suma.api.UserInfo;
...
public class Test {
	...
	public static void main(String[] args) {
		Boolean loginBoolean = login("username", "password");
		if (loginBoolean) {
			getUserInfos();
		}
		else {
	...
	public static Boolean login(String userName, String password) {
		...
		String result = ApiService.login(userInfo);
	...
	public static String getUserInfos() {
		if (userInfo.isLogin()) {
			String result = ApiService.getUserInfo(userInfo.getUid(), userInfo.getToken());
	...
	public static String getMobileNum(String pid) {
		if (userInfo.isLogin()) {
			String result = ApiService.getMobileNum(pid, userInfo.getUid(), userInfo.getToken(), 10);
			System.out.println(result);
			return result;
		}
		return "";
	...
	public static String addIgnore(String mobileNum, String pid) {
		if (userInfo.isLogin()) {
			String result = ApiService.addIgnore(mobileNum, userInfo.getUid(), userInfo.getToken(), pid);
			System.out.println(result);
			return result;
		}
		return "";
	...
	public static String getVcodeAndReleaseMobile(String mobileNum, String author_uid) {
		if (userInfo.isLogin()) {
			String result = ApiService.getVcodeAndReleaseMobile(userInfo.getUid(), userInfo.getToken(), mobileNum, author_uid);
			System.out.println(result);
			return result;
		}
		return "";
	...
	public static String getVcodeAndHoldMobilenum(String mobileNum, String author_uid, String nextId) {
		if (userInfo.isLogin()) {
			String result = ApiService.getVcodeAndHoldMobilenum(userInfo.getUid(), userInfo.getToken(), mobileNum, nextId, author_uid);
			System.out.println(result);
			return result;
		}
		return "";
