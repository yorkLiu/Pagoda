...
oz_fixed_tickets_query='project = "CMC JIRA Tickets" AND labels = %s AND status in (Resolved, Closed) ORDER BY status DESC'
    ...
    'email_content_tpl': """\
         <html>
              ...
              <div>
                  ...
                  <div>OZ Ticket Tracking job will trigger at 8:40 am on Mon - Fri</div>
                  </div>
                </div>
              </body>
         </html>
...
def get_extra_info(oz_ticket_status, cmc_ticket_status, author, oz_ticket_number, cmc_ticket_number):
    extra_text = ''
    p_cmc_ticket_status = str(cmc_ticket_status).replace('(', '').replace(')', '').replace(' ', '-').strip().upper()

    if oz_ticket_status.upper() in ('RESOLVED', 'CLOSED') and p_cmc_ticket_status in ('OPEN-DEV', 'IN-PROGRESS-DEV'):
...
def generate_item_content(oz_ticket, cmc_ticket):
    ...
    oz_ticket_status = oz_jira_status_map[oz_ticket_status_name] or oz_ticket_status_name
